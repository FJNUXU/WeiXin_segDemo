...
def side_mouth(img,intx, inty, intw, inth):
...
def detect_result(img_filename):
    img = face_recognition.load_image_file(img_filename)
    #img=cv2.flip(img,1)#

    face_locations, face_landmarks_list, ratio = resize_and_recongnition(img)#缩小图片并进行face_recongnition算法检测
    ...
    # print("识别的人脸个数:{}".format(faceNum))
    for i in range(0, faceNum):
        top = int(face_locations[i][0] * ratio)
        right = int(face_locations[i][1] * ratio)
        bottom = int(face_locations[i][2] * ratio)
        left = int(face_locations[i][3] * ratio)

    #检测到特征点
    if (len(face_landmarks_list) != 0):
        # img = get_outline(img, face_locations, face_landmarks_list)  #对于检测到特征点的图像，将特征点按五官连线
        img = get_outline(img, ratio, face_landmarks_list)
        # cv_show("fea_img", img)
        img = fill_outline(img)  # 得去除五官的照片
        result_BGR = check_face_yCrCb(img)  # 再去除背景，得正常皮肤与红斑部分
    else:  # 检测不到特征点
        img = check_face_yCrCb(img) #去除背景
...
# img = detect_result("img/cz_0.jpg")
# cv2.imwrite("folder/color_tss.jpg", img)